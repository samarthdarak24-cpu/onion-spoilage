      ...
      {!device ? (
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <MetricTile icon={Wind} label="Ethane" value={reading.ethane} unit="ppm" tone="forest" />
            <MetricTile icon={Flame} label="Methane" value={reading.methane} unit="ppm" tone="forest" />
            <MetricTile icon={Thermometer} label="Temperature" value={reading.temperature} unit="°C" tone="amber" />
            <MetricTile icon={Droplets} label="Humidity" value={reading.humidity} unit="%" tone="amber" />
          </div>
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <Card>
              <div className="mb-2 flex items-center gap-2 font-bold text-ink"><Wind size={16} className="text-fresh" /> Ethane Trend</div>
          ...
          <Card>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 font-bold text-ink"><Activity size={18} className="text-fresh" /> Early Spoilage Risk</div>
              <Badge tone={riskTone[gas?.stage] || 'forest'}>
                {gas?.stage === 'HIGH' && <AlertTriangle size={13} />}{gas?.stage || '—'}
              </Badge>
            </div>
            <p className="mt-2 text-sm text-muted">
              {gas?.stage === 'LOW' && 'Gas signatures are within normal range. No early-spoilage indicators detected.'}
              {gas?.stage === 'MEDIUM' && 'Elevated gas concentration detected — possible early-stage spoilage. Correlate with vision before final grading.'}
              {gas?.stage === 'HIGH' && 'Strong spoilage signature detected. Fusion engine should flag early-spoilage override even if onions look healthy.'}
            </p>
            {gas?.stage && (
...
function MetricTile({ icon: I, label, value, unit, tone }: { icon: any; label: string; value: number; unit: string; tone: 'forest' | 'amber' }) {
  return (
    <div className="card p-4">
      <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-muted">
        <I size={15} className={tone === 'amber' ? 'text-amber-500' : 'text-fresh'} />{label}
      </div>
      <div className="mt-1 flex items-baseline gap-1">
        <span className={`text-3xl font-extrabold ${tone === 'amber' ? 'text-amber-600' : 'text-forest'}`}>{value}</span>
