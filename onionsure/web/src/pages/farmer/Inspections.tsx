...
import { History, ScanLine, Wind, ArrowRight, Search, FileText } from 'lucide-react';
  ...
  return (
    <PageTransition className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="flex items-center gap-2 text-xl font-extrabold text-ink md:text-2xl">
            <History size={22} className="text-fresh" /> My Lots (Central Quality Trail)
          </h1>
          <p className="mt-0.5 text-sm text-muted">All active, inspected and certified lots tied to your Central Lot ID.</p>
        </div>
        <div className="relative">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted" />
          <input className="input pl-9 max-w-xs" placeholder="Search Central Lot ID…" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
      </div>

      {/* Filter Tabs per Section 12 */}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="My Lots" value={<AnimatedNumber value={items.length} />} sub="central records" accent="forest" icon={History} />
        ...
        <StatCard label="Showing" value={<AnimatedNumber value={filtered.length} />} sub="matching filter" accent="forest" icon={Search} />
      </div>

      {error && <div className="rounded-xl border border-reject/20 bg-reject/5 px-4 py-3 text-sm text-reject">{error}</div>}

      {filtered.length === 0 ? (
        <EmptyState title="No lots found" hint="Try changing your search or filter tab." />
      ) : (
              ...
              <Card className="flex h-full flex-col p-4 transition-shadow hover:shadow-card">
                ...
                <div className="mt-4 flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-3">
                    <button className="inline-flex items-center gap-1 font-semibold text-forest hover:underline" onClick={() => nav(`/farmer/report/${i.id}`)}>
                      <FileText size={14} /> View Report
                    </button>
                    <button className="inline-flex items-center gap-1 font-semibold text-muted hover:text-forest hover:underline" onClick={() => nav(`/certificate/${i.id}`)}>
                      Certificate <ArrowRight size={14} />
                    </button>
                  </div>
                </div>
              </Card>
