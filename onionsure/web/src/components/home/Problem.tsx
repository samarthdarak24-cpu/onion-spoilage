...
function HiddenSpoilageVisual() {
  ...
  return (
    <div className="relative aspect-[16/10] overflow-hidden rounded-xl bg-emerald-950">
      ...
      <img
        src="/onions/healthy-extra.jpg"
        alt="Onion that appears healthy on the surface"
        loading="lazy"
        decoding="async"
        className="relative mx-auto h-full w-full object-cover opacity-90"
      />
      ...
      <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-center justify-between gap-2">
        <span className="rounded-md bg-white/12 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.12em] text-white/80 backdrop-blur">
          Surface: clean
        </span>
    ...
    <div className="flex items-center gap-2.5">
      ...
      <div className="w-[46%] shrink-0 rounded-lg border border-dashed border-emerald-900/25 bg-white/70 p-2.5">
        ...
        <div className="space-y-1.5">
          {[80, 62, 74, 50].map((w, i) => (
            <div key={i} className="h-1.5 rounded-full bg-emerald-900/12" style={{ width: `${w}%` }} />
          ))}
        </div>
      ...
      <div className="flex flex-1 flex-col items-center gap-1">
        <div className="relative h-px w-full overflow-hidden bg-emerald-900/15">
          <motion.div
            className="absolute inset-y-0 w-1/3 bg-gradient-to-r from-transparent via-fresh to-transparent"
            initial={{ x: '-100%' }}
            whileInView={reduce ? { x: 0 } : { x: '300%' }}
            viewport={{ once: true }}
            transition={{ duration: 1.6, repeat: Infinity, repeatDelay: 0.5, ease: 'easeInOut' }}
          />
        </div>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-fresh" aria-hidden>
      ...
      <div className="w-[46%] shrink-0 rounded-lg border border-fresh/30 bg-gradient-to-br from-fresh/12 to-white p-2.5 shadow-soft">
        <div className="mb-1.5 flex items-center justify-between">
          <span className="text-[9px] font-bold uppercase tracking-[0.12em] text-forest">Digital</span>
        ...
        <div className="space-y-1.5">
          {[88, 70, 80, 58].map((w, i) => (
            <motion.div
              key={i}
              className="h-1.5 rounded-full bg-fresh/45"
              initial={{ width: 0 }}
              whileInView={{ width: `${w}%` }}
              viewport={{ once: true }}
...
const CARDS = [
  {
    ...
    body: 'Different inspectors can evaluate the same onion batch differently.',
    Visual: SubjectiveVisual,
  },
  {
    n: '02',
    title: 'Hidden Spoilage',
    body: 'Some quality risks may not be visible from the surface.',
    Visual: HiddenSpoilageVisual,
  },
  {
    n: '03',
    title: 'No Digital Evidence',
    body: 'Manual records make verification and dispute resolution difficult.',
    Visual: NoEvidenceVisual,
  },
];

export default function Problem() {
  return (
    <section className="bg-[#FAF8F2] py-20 sm:py-24 lg:py-28" id="problem">
      <div className="mx-auto max-w-[1240px] px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3 lg:gap-6">
          {CARDS.map(({ n, title, body, Visual }, i) => (
            <Reveal key={n} delay={i * 0.12}>
              <article className="group h-full rounded-2xl border border-emerald-900/[0.07] bg-white p-6 shadow-[0_1px_2px_rgba(20,20,25,0.04),0_10px_30px_rgba(20,20,25,0.05)] transition-all duration-300 hover:-translate-y-1.5 hover:border-fresh/35 hover:shadow-[0_18px_44px_-12px_rgba(6,69,44,0.22)]">
                <div className="flex items-baseline gap-3">
                  <span className="font-mono text-[13px] font-bold text-fresh">{n}</span>
                  <span className="h-px flex-1 bg-emerald-900/10" />
                </div>
                <h3 className="mt-4 text-[15px] font-extrabold uppercase tracking-[0.06em] text-emerald-950">
                  {title}
                </h3>
                <p className="mt-2 text-[13.5px] leading-relaxed text-emerald-800/65">{body}</p>
                <div className="mt-5">
                  <Visual />
                </div>
              </article>
